...
def get_parser():
    ...
    parser = argparse.ArgumentParser(
        description="automatically execute some go fluent activities.",
    ...
    group1.add_argument(
        "--simple-run", dest="url", help="the URL of the activity to solve."
    )
    ...
    parser.add_argument(
        ...
        default=True,
        required=False,
        help="Enable the cache system. If enabled, the program will store the done activities in a file.",
        action=argparse.BooleanOptionalAction,
    )
...
def get_credentials(logger: Logger, profile: Optional[str]):
    ...
    if username is None or password is None:
        msg = (
            f"No profile found with the specified profile mathing the following keys : '{username_key}' and"
...
def main():
    ...
    try:
        ...
        logger.error(
            chalk.bold(chalk.red("The program has been stopped by the keyboard."))
        )
        return
